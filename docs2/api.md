...
- `?diaChi=abc`: Khi user nhập từ khóa địa chỉ cần tìm (ví dụ: "Quận 1", "Nguyễn Trãi").
...
const response = await axios.get('/api/phong-tro/search', {
  params: {
    ...
    diaChi: addressKeyword,  // vd: "Quận 1"
...
| PUT | `/hop-dong/{id}/trang-thai` | ✅ Landlord | Duyệt / Từ chối / Phê duyệt hủy hợp đồng |
| PUT | `/hop-dong/{id}/gia-han` | ✅ Landlord | Gia hạn hợp đồng |
| PUT | `/hop-dong/{id}/thanh-ly` | ✅ Landlord | Thanh lý hợp đồng |
| PUT | `/hop-dong/{id}/khach-huy` | ✅ User | Khách thuê gửi yêu cầu hủy hợp đồng (Hợp đồng chuyển thành `YEU_CAU_HUY`) |

**Workflow trạng thái:** `CHO_DUYET` → `DA_DUYET` (Phòng thành `DA_THUE`) hoặc `TU_CHOI`
*Quy trình hủy hợp đồng:* Khách gửi yêu cầu hủy (`khach-huy`) ➡️ Trạng thái chuyển thành `YEU_CAU_HUY` ➡️ Chủ trọ click phê duyệt hủy ➡️ Trạng thái thành `DA_KET_THUC` / `DA_THANH_LY` và giải phóng phòng về `TRONG`.
